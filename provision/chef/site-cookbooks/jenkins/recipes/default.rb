...
# Recipe:: default
#
%w{ java-1.7.0-openjdk java-1.7.0-openjdk-devel }.each do |package|
  package "#{package}" do
    action [ :install ]
  end
end
...
node['jenkins']['plugin'].each do |plugin_id|
  /http:\/\/updates\.jenkins-ci\.org\/download\/plugins\/(.*)\/(.*)\/(.*)\.hpi/ =~ plugin_id
  ...
  execute "install jenkins plugin ( #{plugin_id} )" do
    ...
    EOS
    not_if "java -jar #{node['jenkins']['cli-install-dir']}/jenkins-cli.jar -s http://localhost:#{node['jenkins']['port']} list-plugins | grep #{plugin_name}"
